...
## 6. 任务粒度：用 `mapBatch` 批量合并（v1.13.0 起）

`parallel` 引擎「派发 + 取回」单次固定成本约 4~5 µs；「每任务独立 fork 的多进程」则约 0.3~0.4 ms，
高一个数量级。**`map()` 是逐条派发**（每条序列化一次闭包 + 参数），高频短任务下派发成本会吃掉全部收益。

从 v1.13.0 起用 `mapBatch()` / `mapBatchSettled()`：每批只序列化一次闭包，是高频短任务的默认选择。

```php
// ❌ 5 万条短消息：逐条派发反而比串行慢 5 倍
$pool->map($messages, $render);          // 213k msg/s（串行 1,080k）

// ✅ 批量合并：批大小不传 = 自动推导
$pool->mapBatch($messages, $render);     // 1,655k msg/s，1.53× 串行
```

实测（`benchmarks/bench_batch.php`，11 线程，批 128）：

| 负载 | 串行 | `map` 逐条 | `mapBatch` 批量 | 批量 vs 逐条 |
|------|------|-----------|----------------|-------------|
| trivial（`return $i`，N=40,000） | 28,093,991 ops/s | 250,370 | 5,522,224 | **22.1×** |
| light（2k 次整数循环，N=20,000） | 51,404 ops/s | 145,470 | 293,588 | **2.0×** |
| medium（20 万次 sqrt/sin，N=2,000） | 105 ops/s | 626 | 533 | 0.9×（CPU 密集，靠并发度） |

批大小选择：

- **默认 auto**（不传第三参）：按 `元素数 / (线程数 × 4)` 推导并封顶 1024，短/大数据档实测直接命中全场最优。
...
多用户调用第三方 API 时，`CurlMulti` 的并发度是**最重要的旋钮**（实测 200 请求 × 20ms 延迟）：

| 并发档 | 耗时 | 吞吐 | 相对顺序 |
|-------|------|------|---------|
| 不限（一次 200 连接） | 10019 ms | 20 req/s | **0.4×，比顺序还慢** |
| 8 | 695 ms | 288 req/s | 7.8× |
| **16（最优）** | **451 ms** | **444 req/s** | **8.9×** |
| 32 | 561 ms | 356 req/s | 7.1× |
| 64 | 5073 ms | 39 req/s | 1.1×（对端过载） |

```php
CurlMulti::fetch($urls, concurrency: 16, timeout: 30);
```
...
3. 高频短任务一律 `mapBatch()`（批大小用 auto），别用逐条 `map()`；
4. 进程内计数用**未命名 `Atomic`**（≈16.6M ops/s）；跨进程才用命名 `Atomic`/`Lock`/`Barrier`；
5. 任务参数/返回值尽量小，避免序列化；
6. HTTP 扇出用 `CurlMulti` 并**限并发 8~32**，不要一次全发；
