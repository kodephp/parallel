...
> 版本：`v1.14.0` ｜ kode 栈：`kode/context 3.1.0` / `kode/facade 3.2.0` / `kode/fibers 4.5.0`
> 本次重点：**`map`/`mapSettled` 自动批量合并 + `Futures` 组合器轮询退避（v1.14.0）**，并修复基准可信度与健壮性。
...
因为省掉了 fork 与 IPC；短数据下并行本身收益就有限（串行已 150 万条/秒）。
