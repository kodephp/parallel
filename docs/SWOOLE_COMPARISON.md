...
> kode 侧版本：`v1.15.0` ｜ kode 栈：`kode/context 3.1.0` / `kode/facade 3.2.0` / `kode/fibers 4.5.0`（均为当前各包最新版）。
...
### kode/parallel 实测（本仓库 `benchmarks/bench_concurrency.php` / `bench_compare.php`，ZTS + ext-parallel 真线程主线，v1.15.0）
