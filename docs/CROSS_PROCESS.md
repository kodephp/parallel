...
- 锁/屏障的临时文件回收（v1.18.1 起）：**未命名**的 `Lock` / `Barrier` 用 `tempnam()` 开私有文件，路径不外泄，实例销毁即删除；**具名**的原语文件是跨进程共享入口，绝不随实例销毁而删——删了会让后来者 `tempnam`/重建出另一个 inode，互斥关系就此失效。常驻进程里批量创建未命名锁不再累积 `/tmp/kode_lock_*`。
- 共享计数文件的读失败**不再当成 0**（v1.18.1）：`Atomic` / `Semaphore` 的命名模式下，数据文件读不出来时直接抛 `ParallelException`。旧实现把读失败折成 0，下一次写回就把已有计数静默清零（丢失更新且无从察觉）；文件内容为空串仍按初始值 0 处理。
