...
[![Package Version](https://img.shields.io/badge/Version-1.2.1-orange)](composer.json)
...
| 扩展 | 并发模型 | 内存模型 | 通信方式 |
|------|---------|---------|---------|
| **ext-parallel** | **多线程** | 独立空间(COW) | Channel |
| pthreads | 多线程 | 共享内存 | Volatile |
| pcntl | 多进程 | 完全独立 | IPC/pipe |

**ext-parallel 和 pthreads 都是多线程**，核心区别在于内存模型：
- **ext-parallel**：线程独立内存 + Channel 通信（更稳定）
- **pthreads**：线程共享内存 + Volatile 同步（更复杂）

详见 [PTHREADS_COMPARISON.md](docs/PTHREADS_COMPARISON.md) 和 [PCNTL_COMPARISON.md](docs/PCNTL_COMPARISON.md)
