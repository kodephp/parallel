...
### 单线程 vs 多线程：真实提速（bench_tune，本机 11 核）

「开并行就更快」是误区：**单线程（threads=1）因多了线程 FIFO + 序列化开销，反而比串行略慢（0.8~0.9×）；
真正提速来自多线程（threads≈核数）**。

| 负载 | 串行（参考） | 单线程 threads=1 | 多线程 threads=11（最优批） | 多线程 threads=22 |
|------|------------|-----------------|----------------------|-----------------|
| 中 4KB × 20,000 | 129,232 msg/s | 103,386（**0.8×，比串行慢**） | **672,498（5.4×）** | 631,649（5.1×） |
